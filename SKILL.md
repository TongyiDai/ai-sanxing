...
默认本技能按描述匹配时触发。若想让它在每次"即将收尾交付"时自动自省，可在宿主 Agent 的指令文件（Codex/TRAE/Cursor 的 `AGENTS.md`、Claude Code 的 `CLAUDE.md`）里加一条规则：在输出"已完成/搞定"之前，先按 AI 三省自审一轮。保持可选、幂等、可安全重复。
